...
def get_user(gmail_id: str, db: Session = Depends(get_db)):
...
def get_chats(gmail_id: str, db: Session = Depends(get_db)):
    return get_user_chats(db, gmail_id)

@router.delete("/chats/{gmail_id}/{chat_id}")
def delete_chat(gmail_id: str, chat_id: int, db: Session = Depends(get_db)):
